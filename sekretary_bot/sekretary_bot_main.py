...
from sekretary_bot.sekretary_dp import *
...
@dp.message_handler(Text(equals=['добавить аккаунт','показать задачи','удалить все аккаунты','удалить аккаунт'],
                         ignore_case=True))
...
async def start_mes(message: types.Message, state: FSMContext):
    if message.from_user.id == admin_id:
        await bot.send_message(message.from_user.id, 'сдравствуйте господин админ', reply_markup=admin_kb)
        await FSMAdmin.show_info.set()
    else:
        await bot.send_message(message.from_user.id, f'привет {message.from_user.full_name}я твой секретарь,'
        f'буду напоминать тебе о твоих делах',
...
async def send_inf(message: types.Message, state: FSMContext):
    ...
    elif message.text == 'посмотреть мои задачи':
...
u_s_k_b = ReplyKeyboardMarkup(resize_keyboard=True).add(KeyboardButton('добавить аккаунт'))\
    ...
    .add(KeyboardButton(f'показать мои задачи'))


admin_kb = ReplyKeyboardMarkup(resize_keyboard=True).add(KeyboardButton('добавить аккаунт'))\
    .add(KeyboardButton('удалить аккаунт'))\
    .add(KeyboardButton('удалить все аккаунты'))\
    .add(KeyboardButton(f'показать задачи'))
